...
import {Wallet, Contract, BigNumber, constants } from 'ethers'
...
describe('NoahPair', () => {
  ...
  beforeEach(async () => {
    ...
    const fp = () =>{ return pairFixture([wallet])}
  ...
  it('mint', async () => {
    ...
    await expect(pair.mint(wallet.address, overrides))
      ...
      .to.emit(pair, 'Mint')
      .withArgs(wallet.address, token0Amount, token1Amount, expectedLiquidity.sub(MINIMUM_LIQUIDITY), expectedLiquidity.sub(MINIMUM_LIQUIDITY))
  ...
  it('swap:token1', async () => {
    ...
    await expect(pair.swap(expectedOutputAmount, 0, wallet.address, '0x', overrides))
      ...
      .withArgs(pair.address, wallet.address, expectedOutputAmount)
      .to.emit(pair, 'Sync')
      .withArgs(token0Amount.sub(expectedOutputAmount), token1Amount.add(swapAmount), BigNumber.from("7071067811865475244"))
      .to.emit(pair, 'Swap')
  ...
  it('swap:gas', async () => {
    ...
    expect(receipt.gasUsed).to.eq(85956)
  })
  ...
  it('burn', async () => {
    ...
    await expect(pair.burn(wallet.address, overrides))
      ...
      .to.emit(pair, 'Burn')
      .withArgs(wallet.address, token0Amount.sub(1000), token1Amount.sub(1000), wallet.address, 0, expectedLiquidity.sub(MINIMUM_LIQUIDITY))
